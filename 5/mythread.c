...
void * hello(void* arg)
{
    ...
    while (count < 10) {
        // grab the semaphore
        sem_wait(&sem); // stops here until the value is greater than 0
...
{
    void* status;
    sem_init(&sem, 0, 1); // want to make sure that one of the threads can grab
                          // when it does, it will decrement the value
                          //

    pthread_t tid[2];
    pthread_create(&tid[0], NULL, (void*)hello, (void*)1);
    pthread_create(&tid[1], NULL, (void*)hello, (void*)100);
