 ...
 *  argv[1] holds first cmdline arg
 *  etc.
 *
...
int main( int argc, char * argv[])
{
	...
	if (argc == 4) {  
		...
		ifname = (char *)malloc(strlen(argv[1])+1);  
		strcpy(ifname, argv[1]);  
		if (strncmp(ifname, "stdin", 5)== 0) 
			stdinYES = 1;

		ofname = (char *)malloc(strlen(argv[2])+1);  
	...
	*/
	...
	buf[sz] = '\0';   /* mark the end of a Cstring with \0 */
	...
	/* DEMONSTRATE FILE STREAMS */
	{
		...
		if (fin) {
			...
			fprintf(stderr, "Error - opening **foo**\n");
			exit(1);
		}
	}
	/* demonstrate fstat(2) */
	{
		...
		printf("Infile Links:  \t%ld\n", fileStat.st_nlink);
		printf("Infile inode:  \t%ld\n", fileStat.st_ino);
		printf("Permissions:   \t");
